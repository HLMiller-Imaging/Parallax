function [TopCalibLoc, BottomCalibLoc, FLAG]=CheckCalib2(Pairings,calibtop,calibbottom,nframes)
%Checkcalib2
%this function takes  a set of user defined pairings for bead calibration
%data and then sees which of them have a single location in all the
%frames. Compared to checkcalib checkcalib2 has an additional ability to
%try and make more pairings if no particles tracked the whole way through
% INPUTS
...
%                  calibration
% FLAG - 0 if a trajectory was paired, 1 if it wasn't
...
elseif RowsLeft==0
    disp('There are no suitable candidates for the calibration - returning a flag');
    FLAG=1;
    TopCalibLoc=0;
    BottomCalibLoc=0;
